...
extern SEM_ID semIdShare;
//extern int g_dbg_level = 0;
//extern FILE *g_log_fp = NULL;
...
//    initShareMem();
    ...
    //long memSize = WIDTH*HIGH*4;
    //pData  = (unsigned char*)malloc(memSize);
    //myData = (unsigned char*)malloc(memSize);
    int fd;
...
//        mutexListDecoded->unlock();
        ...
        qDebug("refreshVideo ---1  listsize %d ",listDecoded.size());
         ...
         mutexListDecoded->lock();
         memcpy(myData,pData,WIDTH*HIGH*4);
         mutexListDecoded->unlock();

        QImage img = QImage((uchar *)myData,videoCodecCtx->width,videoCodecCtx->height,
              QImage::Format_RGB32);
...
bool MyFfPlayer::open(QString url, WId wId,bool isLive){
    this->url = url;
    bIsLive = isLive;
    mWid = wId;

    videoLabel = (QLabel*)QLabel::find(wId);
    bQuit = false;
    ...
    QString timeGap = config->value("/system/timeGap").toString();
    if(timeGap == ""){
        TIMEGAP = 30000000;
    }else if(timeGap.toLong() > 100000000){
        TIMEGAP = 30000000;
    }else if(timeGap.toLong() < 20000000){
        TIMEGAP = 30000000;
    }else {
        TIMEGAP = timeGap.toLong();
    }
...
void MyFfPlayer::flushMsgQueue()
{
    int msgNums = msgQNumMsgs(msgId);
    while(msgNums > 0 ){
        MYMSG msg;
...
//            qWarning("listRead size > 10");
        ...
        */
        ...
        MYMSG msg;
        msg.pkt = pkt;
        msgQSend(msgId,(char*)&msg,sizeof(MYMSG),NO_WAIT,MSG_PRI_NORMAL);
           /*
...
//            mutexListDecoded->lock();
//            listDecoded.append(mPic);
            pts = pkt.pts*time_base_num/time_base_den;
//            mutexListDecoded->unlock();
            //mutexListDecoded->lock();
            semTake(semIdShare, WAIT_FOREVER);
            memcpy(pData+64,mPic.data[0],WIDTH*HIGH*4);
            //mutexListDecoded->unlock();
            semGive(semIdShare);
